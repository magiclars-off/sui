    ...
    fn check_duplicates(context: &mut Context, sp!(ploc, pattern): &E::MatchPattern) -> Bindings {
        match pattern {
            ...
            EP::Or(left, right) => {
                ...
                for (key, mut_and_locs) in &left_bindings {
                    ...
                    if lhs_mutability.is_some() && rhs_mutability.is_none()
                        || lhs_mutability.is_none() && rhs_mutability.is_some()
                    {
                        if lhs_mutability.is_none() {
                            report_mismatched_or_mutability(
                                context,
                                rhs_mutability.unwrap(),
                                key.loc(),
                                key,
                                OrPosn::Right,
                            );
