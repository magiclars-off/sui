        ...
        Ok(match self {
            ...
            Argument::Vector(vs) => MoveValue::Vector(
                vs.iter()
                    ...
                    .with_context(|| format!(
                            "Was unable to parse '{self}'. This is most likely because \
                            the vector contains non-primitive (i.e., object) values which aren't permitted."
                    ))?,
            ),
            Argument::String(s) => {
            ...
            Argument::Option(o) => {
                if let Some(v) = o {
                    let v = v.as_ref().into_move_value_opt().with_context(|| {
                        format!(
                            "Was unable to parse '{self}'. This is most likely because \
                            the option contains a non-primitive (i.e., object) value which isn't permitted."
                        )
                    })?;
